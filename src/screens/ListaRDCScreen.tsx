...
const ListaRDCScreen = ({ navigation, route }) => {
  const userName = route.params?.userName || 'Usuário';
  const [rdcs, setRDCs] = useState([]);
  const [refreshing, setRefreshing] = useState(false);

  // Simulação de dados enquanto implementamos o banco
  const loadRDCs = async () => {
    try {
      ...
      Alert.alert('Erro', 'Não foi possível carregar os RDCs.');
    } finally {
      setRefreshing(false);
    }
  };
        ...
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Sucesso', 'RDC excluído com sucesso!');
            } catch (error) {
              console.log('Erro ao excluir RDC:', error);
              Alert.alert('Erro', 'Não foi possível excluir o RDC.');
            }
          },
        },
      ]
    ...
    >
      <View style={styles.rdcInfo}>
        ...
        <ThemedText style={styles.rdcOcorrencias}>
          Ocorrências: {item.total_ocorrencias || 0}
        </ThemedText>
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <ThemedText style={styles.emptySubtext}>
              Toque no botão + para criar um novo RDC
            </ThemedText>
          </View>
        }
      />
